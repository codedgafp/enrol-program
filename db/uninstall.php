 ...
 */
function xmldb_enrol_program_uninstall() {
    global $DB;

    $program = enrol_get_plugin('program');
    $rs = $DB->get_recordset('enrol', ['enrol' => 'program']);
    foreach ($rs as $instance) {
        $program->delete_instance($instance);
    }
    $rs->close();

    role_unassign_all(['component' => 'enrol_program']);
